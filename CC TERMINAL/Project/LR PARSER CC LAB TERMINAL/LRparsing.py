...
def parse_input_string(G, T, Nt, action_list, goto_list, reduction_states, accept_state, input_str):
    stack = [0]
    i, top = 0, 0
    ...
    while True:
        s = stack[top]
        try:
            print(s, stack, input_str[i] if i != len(input_str) else 'Finish', end=' ')
            if s == accept_state:
                print('accept')
                break
...
if __name__ == "__main__":
    ...
    parse_input_string(G, T, Nt, action_list, goto_list, reduction_states, accept_state, input_str)
